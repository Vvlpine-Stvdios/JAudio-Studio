...
| -: | ---------------------------------------------- |
|  0 |  EOF           <sup>[1](#validity-note)</sup>  |
|  1 | [BST](#bst)                                    |
|  2 | [IBNKs](#ibnk)                                 |
|  3 | [WSYSs](#wsys)                                 |
|  4 |  BSC           <sup>[1](#validity-note)</sup>  |
|  5 | [BSM](#bsm)                                    |
|  6 | [SCL](#scl1)   <sup>[1](#validity-note)</sup>  |
|  7 | [SSC](#ssc1)   <sup>[1](#validity-note)</sup>  |
...
| `u32`     | Size                                       |                 |
| `u32`     | Bank ID                                    |                 |
| `u32`     | Bank Version<sup>[1](#validity-note)</sup> |                 |
...
| `(u32 *)[n]` | [INST](#inst) / [PER2](#per2) Pointers<sup>[2](#pointer-note)</sup> | There are at most 243 of these. If there is a specific point where the INST pointers stop and the PER2 pointers begin, I do not know. Ignored when 0. |
...
| `(u32 *)[n]` | [Keyboard Section](#keyboard-section) Pointers<sup>[2](#pointer-note)</sup> | One pointer for each of the `n` keyboard sections |
...
| `u32 *` | [Attack](#vector-table) Pointer<sup>[2](#pointer-note)  | The pointer to the [vector table](#vector-table) holding the information for the attack/sustain/decay   |
| `u32 *` | [Release](#vector-table) Pointer<sup>[2](#pointer-note) | The pointer to the [vector table](#vector-table) holding the information for the release                |
| `f32`   | Width                                                   | Modulation depth                                                                                        |
| `u32`   | Vertex Count                                            | Populated by engine at run time<sup>[1](#validity-note)                                                 |
| `u8[8]` | Padding                                                 |                                                                                                         |
...
| `(u32 *)[n]` | [Velocity Region](#velocity-region) Pointers<sup>[2](#pointer-note)</sup> | One pointer for each of the `n` velocity regions.                         |
...
### PER2
PER2's (Percussion [no idea what the 2's for]) work the same way as INST structures, except they remove unneeded information.

|      Type      |                               Name                                |                    Description                    |
| -------------- | ----------------------------------------------------------------- | ------------------------------------------------- |
| `char[4]`      | Magic                                                             | 'PER2' in ASCII                                   |
| `(u32 *)[128]` | [Drum Region](#drum-region) Pointers<sup>[2](#pointer-note)</sup> | One for each key of the keyboard. Ignored when 0. |
...
| `u32`     | ID                                                 |                 |
| `u32`     | Wave Count                                         |                 |
| `u32 *`   | [WINF](#winf) Pointer<sup>[2](#pointer-note)</sup> |                 |
| `u32 *`   | [WBCT](#wbct) Pointer<sup>[2](#pointer-note)</sup> |                 |
...
| `(u32 *)[n]` | [Wave Group](#wave-group) Pointers<sup>[2](#pointer-note)</sup> | One pointer for each of the `n` Wave Groups. |
...
| `u32`        | Wave Count                                          | `n`                                                                     |
| `(u32 *)[n]` | [WAVE](#wave) Pointers<sup>[2](#pointer-note)</sup> | One pointer for each of the `n` WAVEs                                   |
...
| `(u32 *)[n]` | Pointers<sup>[2](#pointer-note) | There are two pointers here. Each points to an array of 16 `0x04`'s. According to AI this means a max of 4 voices per category. |

## SSC<sup>[1](#validity-note)</sup>
...
<div style="font-size: 8pt; padding-bottom: 5px;"><a id="validity-note"><strong>[1]</strong></a>: <em>This is not verified; it was simply what an AI told me an it may or may not be accurate.</em></div>
<div style="font-size: 8pt; padding-bottom: 5px;"><a id="pointer-note"><strong>[2]</strong></a>: <em>All pointers inside major structures defined at the beginning are offset relative to that structure's header.</em></div>
